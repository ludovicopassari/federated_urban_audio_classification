...
def iid_partitioning(metadata_df):
    ...
    # Salvataggio su CSV
    output_file = Path(dataset_dir / "partitioning-metadata/urban8k_iid_distribution.csv")
...
def dirichlet_partitioning(metadata_df, alpha: float = 0.5):
    ...
    # Distribuzione dei dati per classe
    for class_label, indices in class_indices.items():
    ...
    # Salvataggio su CSV
    output_file = Path(dataset_dir / "partitioning-metadata/urban8k_dirichlet_distribution.csv")
    output_file.parent.mkdir(parents=True, exist_ok=True)
